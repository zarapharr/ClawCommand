...
export function Sidebar({ currentView, onNavigate, isOpen, featureAvailability }: SidebarProps) {
  return (
    <aside 
      className={cn(
    ...
    >
      ...
      >
        {navItems.map((item) => {
          ...
          const isDisabled = (item.id === 'voice' && featureAvailability && !featureAvailability.voice.enabled)
            || (item.id === 'qmd' && featureAvailability && !featureAvailability.qmd.enabled);
          ...
          return (
            <button
              ...
              className={cn(
                ...
                isDisabled && 'opacity-45 cursor-not-allowed hover:bg-transparent',
