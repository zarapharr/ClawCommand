...
export function setupRestProxies(app: Express) {
  // Langfuse proxy
  app.get('/api/proxy/langfuse/traces', async (req: Request, res: Response) => {
  ...
  app.get('/api/proxy/langfuse/observations', async (req: Request, res: Response) => {
    const { page = 1, limit = 100 } = req.query;
  ...
  app.get('/api/proxy/langfuse/scores', async (req: Request, res: Response) => {
    const { page = 1, limit = 100 } = req.query;
  ...
  // Ollama proxy
  app.get('/api/proxy/ollama/tags', async (_req: Request, res: Response) => {
    const result = await proxyRequest(`${OLLAMA_URL}/api/tags`, 'GET');
    res.status(result.status).json(result.data);
  });

  app.post('/api/proxy/ollama/generate', async (req: Request, res: Response) => {
  ...
  app.get('/api/github/repos', async (req: Request, res: Response) => {
    const { owner = 'zarapharr' } = req.query as { owner?: string };
  ...
  app.get('/api/github/workflows', async (req: Request, res: Response) => {
    const { owner = 'zarapharr', repo = 'ClawCommand' } = req.query;
