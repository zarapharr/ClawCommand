...
class GatewayProxyManager {
  ...
  private queueMessage(type: 'text' | 'binary', data: string | Buffer) {
  ...
  private flushPendingMessages() {
    if (!this.gatewayWs || this.gatewayWs.readyState !== WebSocket.OPEN || !this.authenticated) return;
    while (this.pendingMessages.length > 0) {
      const msg = this.pendingMessages.shift();
      if (msg) this.gatewayWs.send(msg.data);
    }
  }

  private broadcastToClients(data: Buffer) {
...
export function setupGatewayProxy(server: HTTPServer) {
