...
const WorkflowNodeComponent: React.FC<{
  ...
  onPortClick: (e: React.MouseEvent, nodeId: string, port: 'input' | 'output') => void;
}> = ({ node, isSelected, isConnecting, onSelect, onDragStart, onPortClick }) => {
...
export default function WorkflowPage() {
  ...
  const handleCreateWorkflow = () => {
    ...
    const newWorkflow = createWorkflow({
      ...
      pattern: 'sequential',
      nodes: [
        { id: `node-${Date.now()}`, type: 'input', position: { x: 150, y: 250 }, config: {} },
        { id: `node-${Date.now() + 1}`, type: 'output', position: { x: 550, y: 250 }, config: {} },
      ],
      edges: [
        { id: `edge-${Date.now()}`, source: `node-${Date.now()}`, target: `node-${Date.now() + 1}` },
      ],
      isActive: false,
    });
  ...
  const handleAddNode = (type: NodeType) => {
    ...
    const x = 200 + (existingNodes.length * 50) % 300;
    const y = 200 + Math.floor((existingNodes.length * 50) / 300) * 100;
    
    const newNode = addNode(selectedWorkflow.id, {
      type,
      position: { x, y },
      config: type === 'agent' || type === 'supervisor' ? { agentId: '', agentName: '' } : {},
    });
  ...
  const handlePortClick = (e: React.MouseEvent, nodeId: string, port: 'input' | 'output') => {
    e.stopPropagation();
    ...
    if (!isConnecting) {
      ...
      if (connectionStart.nodeId !== nodeId) {
        const sourceId = connectionStart.port === 'output' ? connectionStart.nodeId : nodeId;
        const targetId = connectionStart.port === 'output' ? nodeId : connectionStart.nodeId;
        
        // Check if connection already exists
        const exists = selectedWorkflow?.edges.some(
          e => e.source === sourceId && e.target === targetId
        );
        
        if (!exists && selectedWorkflow) {
          addEdge(selectedWorkflow.id, { source: sourceId, target: targetId });
        }
      }
