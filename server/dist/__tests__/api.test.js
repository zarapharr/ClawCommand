...
describe('ClawCommand Backend API', () => {
    ...
    describe('GitHub Proxy', () => {
        ...
        it('should proxy github issues request', async () => {
            const response = await request(app).get('/api/github/issues?owner=zarapharr&repo=ClawCommand');
            expect(response.status).toBeGreaterThanOrEqual(200);
            expect(response.status).toBeLessThan(600);
        });
    });
    ...
    describe('Audit Logs', () => {
        ...
        it('should filter audit logs by agent', async () => {
            const response = await request(app).get('/api/audit/logs?limit=10&agent=ops-builder');
            expect(response.status).toBeGreaterThanOrEqual(200);
            if (response.status === 200 && response.body.entries.length > 0) {
                const allMatch = response.body.entries.every((e) => e.agent?.includes('ops-builder'));
