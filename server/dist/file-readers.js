...
export function setupFileReaders(app) {
    app.get('/api/audit/logs', async (req, res) => {
        try {
            ...
            for (const file of files) {
                ...
                if (allEntries.length > limitNum + offsetNum)
                    break;
            }
    ...
    app.get('/api/cron/jobs/:jobId', async (req, res) => {
        try {
            ...
            const jobsData = JSON.parse(fs.readFileSync(CRON_JOBS_PATH, 'utf8'));
            const job = jobsData.schedules?.find((j) => j.id === jobId);
            if (!job) {
